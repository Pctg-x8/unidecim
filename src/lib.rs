...
impl DecimatingProcessor
{
    ...
    {
        unsafe { drop(Box::from_raw(state.effect_data::<Self>())) };
        state.write_effect_data::<Self>(std::ptr::null_mut());
        CallbackResult::Ok
    }
    pub fn process(state: &mut UnityAudioEffectState, inbuf: &[c_float], outbuf: &mut [c_float], channels: c_int) -> CallbackResult
    {
        let this = state.effect_data_mut::<Self>();
        for (i, ref mut o) in outbuf.iter_mut().enumerate()
        {
            if this.phase == 0 { this.locking = inbuf[i]; this.div = this.div_pending; }
            o = this.locking;
            this.phase += 1;
    ...
    {
        let this = state.effect_data_mut::<Self>();
        if index == 0 { *value = this.div_pending; CallbackResult::Ok }
        else { CallbackResult::Unsupported }
    }
}
...
impl Parameter
{
    pub fn new(name: &'static str, range: Range<c_float>) -> Self
    {
        ...
        def.name[..name.len()] = name.as_bytes();
        Parameter(def)
    }
    pub fn description(self, desc: &'static str) -> Self
    {
        self.0.description = desc.as_ptr(); self
    }
}
impl Into<UnityAudioParameterDefinition> for Parameter { fn into(self) -> UnityAudioParameterDefinition { self.0 } }

#[repr(C)] pub enum CallbackResult { Ok = UNITY_AUDIODSP_OK, Unsupported = UNITY_AUDIODSP_ERR_UNSUPPORTED }
...
extern "C" fn process_decimating_processor(state: *mut UnityAudioEffectState, inbuf: *mut c_float, outbuf: *mut c_float, length: c_uint, inchannels: c_int, outchannels: c_int) -> UnityAudioDSPResult
{
    ...
    let inbuf = unsafe { std::slice::from_raw_parts(inbuf, length * inchannels) };
    let outbuf = unsafe { std::slice::from_raw_parts_mut(outbuf, length * outchannels) };
    DecimatingProcessor::process(inbuf, outbuf, inchannels) as _
}
...
{
    let r = DecimatingProcessor::get_float_parameter(unsafe { &mut *state }, index, value) as _;
    if !valuestr.is_null() { *valuestr = 0; } r
}
...
pub extern "C" fn UnityGetAudioEffectDefinitions(defptr: *mut *mut *mut UnityAudioEffectDefinition) -> c_int
{
    lazy_static!(
        static ref PARAMS: [UnityAudioParameterDefinition; 1] = [
            Parameter::new("Division Rate", 1 .. 44100).description("Division Rate").into()
        ];
        static ref adef = UnityAudioEffectDefinition
        {
            structsize: std::mem::size_of::<UnityAudioEffectDefinition>() as _,
            paramstructsize: std::mem::size_of::<UnityAudioParameterDefinition>() as _,
            ...
            numparameters: PARAMS.len(), paramdefs: PARAMS.as_ptr(),
            .. Default::default()
        };
        static ref DEFPTRS: [*mut UnityAudioEffectDefinition; 1] = [&adef as *mut _];
    );
    *defptr = DEFPTRS.as_ptr(); 1
}
